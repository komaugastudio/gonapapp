...
const LanguageScreen = () => {
  ...
  const handleSelectLanguage = (code) => {
    console.log('🌍 Mengubah bahasa ke:', code);
    // Instantly change language in context
    setLanguage(code);
    setShowSuccess(true);
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="bg-white p-4 shadow-sm flex items-center">
        ...
        <h2 className="flex-1 text-center font-bold text-lg">{useTranslation(language).t('selectLanguage')}</h2>
      </div>
      ...
      {showSuccess && (
        <div className="fixed top-10 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-top-4 fade-in">
          <div className="bg-green-600 text-white px-6 py-3 rounded-full shadow-lg text-sm font-semibold flex items-center gap-2 whitespace-nowrap">
            ...
            {useTranslation(language).t('languageChanged')}
          </div>
        </div>
      )}
        ...
        {languages.map((lang) => (
          <div
            ...
            className={`flex items-center justify-between p-4 rounded-lg cursor-pointer border-2 transition-all ${
              language === lang.code ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300'
            }`}
          >
            ...
            {language === lang.code && <Check size={24} className="text-green-600" />}
          </div>
        ))}
      </div>
    </div>
